  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <Link
          href={`/dashboard/restaurant/new${demoParam}`}
          ...
          {restaurants.map((restaurant: any) => (
            <div key={restaurant.id} className="bg-white rounded-lg shadow p-4 sm:p-6">
              ...
              <div className="flex flex-wrap gap-2">
                <Link
                  href={`/dashboard/menu?restaurant=${restaurant.id}${isDemo ? "&demo=true" : ""}`}
                  className="bg-orange-500 text-white px-3 py-2 rounded hover:bg-orange-600 text-sm"
                >
                  Gérer le menu
                </Link>
                <Link
                  href={`/dashboard/orders?restaurant=${restaurant.id}${isDemo ? "&demo=true" : ""}`}
                  className="bg-blue-500 text-white px-3 py-2 rounded hover:bg-blue-600 text-sm"
                >
                  Commandes
                </Link>
                <Link
                  href={`/r/${restaurant.slug}${demoParam}`}
                  target="_blank"
